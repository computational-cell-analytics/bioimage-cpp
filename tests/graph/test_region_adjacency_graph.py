...
def test_region_adjacency_graph_2d():
    labels = np.array(
        [
            [1, 1, 2],
            [1, 3, 2],
...
def test_region_adjacency_graph_3d():
    labels = np.array(
        [
            [[1, 1], [2, 2]],
            [[1, 3], [2, 3]],
    ...
    rag = bic.graph.region_adjacency_graph(labels, number_of_threads=1)
...
def test_region_adjacency_graph_parallel_matches_single_thread():
    labels = np.array(
        [
            [0, 1, 1, 2],
            [0, 0, 3, 2],
    ...
    single_threaded = bic.graph.region_adjacency_graph(
        labels, number_of_threads=1
    )
    parallel = bic.graph.region_adjacency_graph(labels, number_of_threads=3)

    np.testing.assert_array_equal(parallel.uv_ids(), single_threaded.uv_ids())


def test_region_adjacency_graph_accepts_non_contiguous_input():
    labels = np.array(
        [
            [1, 9, 2],
            [1, 9, 2],
...
def test_region_adjacency_graph_rejects_invalid_input():
    with pytest.raises(ValueError, match="2D or 3D"):
        bic.graph.region_adjacency_graph(np.ones((1,), dtype=np.uint64))

    with pytest.raises(TypeError, match="labels must have one of dtypes"):
        bic.graph.region_adjacency_graph(np.ones((2, 2), dtype=np.uint8))

    with pytest.raises(ValueError, match="negative"):
        bic.graph.region_adjacency_graph(
