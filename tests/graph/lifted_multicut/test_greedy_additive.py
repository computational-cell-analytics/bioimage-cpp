...
def test_greedy_additive_respects_weight_stop(chain_with_lifted):
    ...
    labels = bic.graph.lifted_multicut.LiftedGreedyAdditiveMulticut(weight_stop=3.0).optimize(objective)
...
def test_greedy_additive_matches_multicut_when_no_lifted_edges():
    ...
    mc_labels = bic.graph.multicut.GreedyAdditiveMulticut().optimize(mc_objective)

    lmc_objective = bic.graph.lifted_multicut.LiftedMulticutObjective(base, base_costs)
    lmc_labels = bic.graph.lifted_multicut.LiftedGreedyAdditiveMulticut().optimize(lmc_objective)
